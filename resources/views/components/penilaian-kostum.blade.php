<div class="flex flex-col gap-2 mb-4">
    ...
    <div class="flex md:flex-row flex-col gap-2 md:items-center items-start justify-between">
        ...
        <div id="keindahan_kerapihan" class="flex flex-wrap gap-4 bg-transparent p-1 px-2 rounded md:text-base text-xs">
            <?php
            $values = [0, 2, 3, 4, 5, 6, 7, 8, 9];
            foreach ($values as $value):
                $paddingClass = $value < 10 ? 'md:px-3 px-2' : 'md:px-2 px-1';
            ...
            <div>
                ...
                <label for="keindahan_kerapihan_<?= $value ?>" class="block border border-gray-500 cursor-pointer select-none md:rounded-xl rounded-md p-1 <?= $paddingClass ?> text-center peer-checked:bg-blue-500 peer-checked:font-bold peer-checked:text-white transition-all"><?= $value ?></label>
            </div>
            <?php endforeach; ?>
        </div>
    </div>
